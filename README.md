...
따라 적용할 이미지와 반복 패턴이 결정됩니다.

![좌측 구역과 배터리 아이템](img/maze2.png)

좌측 구역. 구역마다 다른 텍스처를 지정해 플레이어가 현재 어느 구역에 있는지 시각적으로
구분되게 했습니다. 오른쪽 벽 앞의 금색 오브젝트가 배터리 아이템(`battery.dat`, 정점 270개)
으로, 세 구역에서 3개를 모두 모으면 중앙 정면의 잠금 장치가 해제됩니다.

![구역을 잇는 통로](img/maze3.png)

중앙 구역에서 각 구역으로 이어지는 통로입니다. 벽을 위아래 두 장으로 나누고 그 사이를
비워 바깥의 우주 공간이 보이도록 구성했습니다. 통로 밖의 파란 행성과 크림색 행성은
SOR 모델러로 제작한 `planet1.dat`, `planet3.dat`입니다.

## 실행

| 파일 | 설명 | 빌드 |
| --- | --- | --- |
| `bin/Final.exe` | 미로 게임 본편 | 리팩터링 반영 Release 빌드 |
| `bin/SOR.exe` | 회전체 모델러 | 2023년 원본 (소스 유실) |
...
| 메뉴 | 동작 |
| --- | --- |
| `Print Clicked Points` | 저장된 정점 좌표를 콘솔에 출력 |
| `Scale` | 정점 좌표를 실수 배율로 확대·축소 |
| `Rotate` | 360의 약수를 입력받아 해당 각도씩 y축 회전, 회전체 정점군 생성 |
| `Connect` | 생성된 면 순서대로 정점을 연결해 결과 폴리곤 미리보기 |
| `Clear` | 정점과 면을 모두 초기화 |
| `Make Faces` | 정점 인덱스 3개 단위의 면(Face) 구조체 배열 생성 |
...
![정점 생성과 회전](img/SOR_rotate.png)

사분면 화면을 클릭해 프로파일 정점을 찍으면 콘솔에 좌표가 기록됩니다. `Rotate`를
선택하고 회전 각도를 입력하면(여기서는 60도 — 360의 약수) 그 각도씩 y축을 기준으로
회전한 정점군이 생성됩니다.

![면 생성과 폴리곤 미리보기](img/SOR_polygon.png)

`Make Faces`로 정점 인덱스를 3개 단위의 면으로 묶고 `Connect`로 연결한 결과입니다.
이 상태에서 `Save`를 누르면 `.dat`로 출력되며, 게임 측 `MapObject::readModel()`이
같은 포맷을 읽습니다. 화면의 형상이 최종 목표 오브젝트인 `trophy.dat`입니다.
...
`bin/Final.exe`는 위 수정이 **모두 반영된 Release 빌드**입니다 (x86 / VS2022 v145 / vcpkg).
2023년 제출 당시의 원본 빌드는 첫 커밋 `ff6c370`에서 확인할 수 있습니다.

`bin/SOR.exe`는 소스가 유실되어 2023년 빌드 그대로이며, `bin/freeglut.dll`은 새 빌드와
버전을 맞추기 위해 vcpkg 배포판으로 교체했습니다. 두 실행 파일 모두 이 DLL에서 동작을
확인했습니다.
